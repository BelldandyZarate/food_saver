                    ...
                    filteredProductos.map(producto => (
                        <div key={producto.id} className={`user-card ${isExpired(producto.expiryDate) ? 'expired' : ''}`}>
                            <div className="user-info">
                                {/* Mostrar imagen del producto (si existe en base64) */}
                                {producto.imagePreview ? (
                                    <center><img src={producto.imagePreview} alt={producto.name} className="product-image" /></center>
                                ) : (
                                    <p>No hay imagen disponible</p>
                                )}
                                <h3 className='producname'>{producto.name}</h3>
            ...
            {selectedProducto && (
                <div className="modal">
                    <div className="modal-content">
                        ...
                        {/* Mostrar imagen del producto en detalles (si existe en base64) */}
                        {selectedProducto.imagePreview ? (
                            <center><img src={selectedProducto.imagePreview} alt={selectedProducto.name} className="product-image" /></center>
                        ) : (
                            <p>No hay imagen disponible</p>
                        )}
